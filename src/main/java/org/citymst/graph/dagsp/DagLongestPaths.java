...
public class DagLongestPaths {
    ...
    public DagLongestPaths(Graph dag, List<Integer> topo, Metrics metrics) {
        this.dag = dag;
        this.topo = topo;
    ...
    public static class Result {
        ...
        public int findCriticalNode() {
            int maxVal = Integer.MIN_VALUE, node = -1;
            for (int i = 0; i < dist.length; i++)
                if (dist[i] > maxVal) { maxVal = dist[i]; node = i; }
            return node;
        }
    }
}